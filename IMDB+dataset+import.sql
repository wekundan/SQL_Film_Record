...
union all
select 'movie' as table_name, count(*) as raw_count from movie
union all
...
SELECT column_name AS null_columns
FROM (
   SELECT 'id' AS column_name, id FROM movie WHERE id IS NULL
    UNION
    SELECT 'title' AS column_name, title FROM movie WHERE title IS NULL
    UNION
    SELECT 'year' AS column_name, year FROM movie WHERE year IS NULL
    UNION
    SELECT 'date_published' AS column_name, date_published FROM movie WHERE date_published IS NULL
    UNION
    SELECT 'duration' AS column_name, duration FROM movie WHERE duration IS NULL
    UNION
    SELECT 'country' AS column_name, country FROM movie WHERE country IS NULL
    UNION
    SELECT 'worldwide_gross_income' AS column_name, worlwide_gross_income FROM movie WHERE worlwide_gross_income IS NULL
    UNION
    SELECT 'languages' AS column_name, languages from movie where languages is null
    UNION
    SELECT 'production_company' AS column_name, production_company FROM movie WHERE production_company IS NULL
) AS subquery;

-- table containing null columns are country, worldwid
...
movie_summary AS (
    SELECT
        ...
        CAST(REPLACE(REPLACE(IFNULL(m.worlwide_gross_income, 0), 'INR', ''), '$', '') AS DECIMAL) AS worldwide_gross_income,
        DENSE_RANK() OVER (PARTITION BY m.year, g.genre ORDER BY CAST(REPLACE(REPLACE(IFNULL(m.worlwide_gross_income, 0), 'INR', ''), '$', '') AS DECIMAL) DESC) AS movie_rank
    FROM
        movie AS m
